...
class PlaylistsService {
  ...
  async verifyAccess(playlistId, userId) {
    // owner?
    const { rowCount } = await pool.query('SELECT 1 FROM playlists WHERE id=$1 AND owner=$2', [playlistId, userId]);
    if (rowCount) return true;
    // collaborator?
    const collab = await pool.query('SELECT 1 FROM collaborations WHERE playlist_id=$1 AND user_id=$2', [playlistId, userId]);
  ...
  async delete(playlistId, userId) {
    // hanya owner
    const { rowCount } = await pool.query('DELETE FROM playlists WHERE id=$1 AND owner=$2', [playlistId, userId]);
